...
This lecture covers material from \S 24 of the text \citep{desai2009quantum}, and \S 8 of \citep{griffiths2005introduction}.
...
\imageFigure{../figures/phy456-qmII/qmTwoL12fig1}{Finite well potential.}{fig:qmTwoL13:qmTwoL12fig1}{0.15}
With
%
...
\end{equation}
%
the bound state within the well is approximately
%
\begin{equation}\label{eqn:qmTwoL12:50}
U \propto e^{\pm i k x},
\end{equation}
%
and outside of the well is of the form
%
...
\end{equation}
or
\begin{equation}\label{eqn:qmTwoL12:430}
\begin{aligned}
\phi'(x)
&= \pm \sqrt{ + i k'(x) + k^2(x) } \\
...
\begin{equation}\label{eqn:qmTwoL12:450}
\begin{aligned}
\phi(x)
...
\begin{equation}\label{eqn:qmTwoL12:470}
\begin{aligned}
i \phi(x)
&= - i \int dx \lr{ k(x) - i \frac{k'(x)}{2 k(x)} } \\
...
\(V(x)\) change very slowly. This implies that \( k'(x)\) is small, and \(k(x) = \sqrt{2 m (E - V(x))}/\Hbar\).
We need \(E\) very far away from the potential.  In particular, \(\Abs{(E - V(x))/V(x)} \gg 1\).
%
...
%FIXME: fig3
\imageFigure{../figures/phy456-qmII/qmTwoL12fig3}{Turning points where WKB will not work.}{fig:qmTwoL13:qmTwoL12fig3}{0.15}
